...
var readInputTimeout = null;
// Error handler
function exit (error) {
  clearTimeout(readInputTimeout);
...
function readInput() {
  ...
  system.stdout.write(JSON.stringify({ event: 'newInput' }));
...
page.onLoadStarted = function() {
  ...
  planForcedExit();
  system.stdout.write(JSON.stringify({ event: 'onLoadStarted', data: timeout }))
};
...
page.onLoadFinished = function () {
  ...
  system.stdout.write(JSON.stringify({ event: 'onLoadFinished', filename: filename }))
  readInputTimeout = setTimeout(readInput, 10);
};
