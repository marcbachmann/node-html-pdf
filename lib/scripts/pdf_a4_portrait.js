...
function createSection (section, content, options) {
  ...
  function pageEquals(value, pageNum, numPages) {
    switch (value) {
      case "first": return pageNum == 1
      case "last": return pageNum == numPages
      default: return pageNum == value
    }
  }

  function isHidden(hideOption, pageNum, numPages) {
    if (hideOption instanceof Array) {
      return hideOption.some(function(el) {
        return pageEquals(el, pageNum, numPages)
      })
    } else {
      return pageEquals(hideOption, pageNum, numPages)
    }
  }
  ...
  return {
    ...
    contents: phantom.callback(function (pageNum, numPages) {
      if (isHidden(options.hide, pageNum, numPages)) return ''

      var html = o[pageNum] || c[pageNum]

      if (pageEquals("first", pageNum, numPages) && !html)
        html = o.first || c.first

      if (pageEquals("last", pageNum, numPages) && !html)
        html = o.last || c.last

      return (html || o.default || c.default || '')
        .replace(/{{page}}/g, (pageNum + paginationOffset))
        .replace(/{{pages}}/g, (numPages + paginationOffset)) + content.styles
    })
  }
}
